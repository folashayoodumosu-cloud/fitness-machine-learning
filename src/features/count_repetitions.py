...
from src.features.build_features import LowPass
...
[columns + '_lowpass']
bench_set[columns + '_lowpass'].plot()
# --------------------------------------------------------------
# Create function to count repetitions
# --------------------------------------------------------------

def count_reps(dataset, cutoff=0.4, order=10, columns='acc_r'):
    ...
    dataset[columns + '_lowpass'].plot()
    indexes = argrelextrema(dataset[columns + '_lowpass'].values, np.greater)[0]
    peaks = dataset.iloc[indexes]
    
    figure, ax = plt.subplots()
    plt.plot(dataset[columns + '_lowpass'])
    plt.plot(peaks[columns + '_lowpass'], 'o', color='red')
...
count_reps(row_set, cutoff=0.65, columns='gyr_x')
...
for s in df['set'].unique():
    ...
    reps = count_reps(subset, cutoff=cutoff, columns=column)
